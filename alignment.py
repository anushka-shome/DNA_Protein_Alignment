...
def output(dna, protein, protein_dna, aseq1, aseq2, p_name, d_name):
  ...
  print("Alignment [given (g) vs. translated (t)]:")
...
def main():
  ...
  if len(sys.argv) != 3 and len(sys.argv) != 5:
    print("Usage: python alignment.py <protein_sequences>.fasta <dna_sequences>.fasta [gap_open_penalty] [gap_extend_penalty]")
    return
  elif len(sys.argv) == 5:
    go = int(sys.argv[3])
    gp = int(sys.argv[4])
